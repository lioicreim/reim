...
function generateCurrencyRules(
  ...
  currencyTierList.forEach((tier) => {
    // 선택된 최소 티어보다 낮은(숫자가 큰) 티어는 생성하지 않음
    const currentTierKey = tierMapping[tier] || tier;
    if (tierOrder[currentTierKey] > minTierVal) return;
        ...
        if (tierKey === "S") {
            if (setting?.enabled !== false) {
               ...
               if (useIngame) {
                   ...
                   const rawFile = setting?.pcFile || "custom_sound/1_currency_s.mp3";
                   const file = rawFile.startsWith("custom_sound/") ? rawFile : `custom_sound/${rawFile}`;
                   lines.push(`  CustomAlertSound "${file}" ${setting?.volume || 300}`);
               }
            }
        }
      } else {
         // 일반 처리
         // PS5 혹은 설정이 Default면 PlayAlertSound (기본 매핑 fallback 포함)
         // 단, setting이 null이어도 PS5면 Fallback이 돌아가야 함.
         // 설정이 명시적으로 존재하고 type이 default면 Ingame Sound 사용.
         
         const useIngame = isPS5 || (setting?.type === "default");
         
         if (useIngame) {
             if (setting && setting.enabled !== false) {
                 const slot = setting.ingameId || setting.ps5Slot;
                 if (slot) {
                     lines.push(`  PlayAlertSound ${slot} ${setting.volume || 300}`);
                 } else if (isPS5) {
                     // 설정은 있는데 슬롯이 없는 경우? (보통 없음)
                     // PS5 기본 Fallback logic 필요하면 추가
                 }
             } else if (!setting && isPS5 && ["S", "A", "B", "C"].includes(tierKey)) {
                  // PS5이고 설정이 없을 때 기본 Fallback
             ...
             if (setting?.enabled !== false) {
                 const rawFile = setting?.pcFile || tierColors.customSound;
                 // tierColors.customSound가 있어도 setting이 있으면 setting.pcFile이 우선일 것.
                 // setting.pcFile이 undefined면? -> tierColors.customSound
...
function generateBaseItemsSocketQualityRules(lines, settings, soundContext, lang = "ko") {
  ...
  settings.rules.forEach((rule) => {
    ...
    // 소켓 조건 (소켓 규칙)
    if (isSocketRule && rule.conditions?.sockets) {
      const { operator, value } = rule.conditions.sockets;
      lines.push(`  Sockets ${operator} ${value}`);
    }

    // 퀄리티 조건 (퀄리티 규칙)
    if (isQualityRule && rule.conditions?.quality) {
...
function generateExpeditionRules(lines, expeditionSettings, soundContext, lang = "ko") {
  ...
  rules.forEach((rule) => {
    ...
    if (rule.styles) {
      ...
      if (styles.customSound) {
        const cs = styles.customSound.startsWith("custom_sound/") ? cs : `custom_sound/${cs}`;
        lines.push(`  CustomAlertSound "${cs}" 300`);
      }
    }
